...
namespace Yodo1.Editor
{
    public class VersionUploader
    {
        ...
        const string K_CMD_PUSH_VERSION = "curl -T \"{0}\" -u \"{1}\" \"https://yodo1-generic.pkg.coding.net/anti-indulged-system-SDK/unity-plugin/version.json?version=latest\"";
        const string K_CMD_PUSH_BUILD = "curl -T \"{0}\" -u \"{1}\" \"https://yodo1-generic.pkg.coding.net/anti-indulged-system-SDK/unity-plugin/Yodo1AntiAddictionSDK?version={2}\"";
        
        const string K_VERSION_FILE_PATTERN = "{\"version\":\"#\"}";
        static string RootPath { get { return Directory.GetParent(Application.dataPath).FullName; }}
        static string LibraryPath { get { return Path.Combine(RootPath, "Library");}}
