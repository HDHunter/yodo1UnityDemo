...
namespace Yodo1.AntiAddiction
{
    ...
    public class Yodo1U3dSettingsEditor : UnityEditor.Editor
    {
        ...
        public static string PodAssetPath { get { return "Assets/" + Yodo1U3dAntiAddictionEditor.K_SDK_ROOT_NAME + "/bin/" + K_SDK_POD_PATH; } }
        public static string DependenciesAssetPath { get { return "Assets/" + Yodo1U3dAntiAddictionEditor.K_SDK_ROOT_NAME + K_SDK_DEPENDENCIES_PATH; } }
        ...
        public static void MergeAntiAddictionPodfile(string podPath)
        {
            ...
            if (!string.IsNullOrEmpty(cocoapodVersion))
            {
                ...
                {
                    Debug.LogFormat("<color=#00ff00>>> inject pod for Anti-Addiction: total {0} lines</color>", pods.Length);
        ...
        public static void UpdateDependencies(string version)
        {
            if (File.Exists(DependenciesAssetPath))
            {
                ...
                {
                    Debug.LogFormat("<color=orange>[!] Can find node [{0}] in \n{1} </color>", nodePath, DependenciesAssetPath);
                }

                if (found)
                {
                    Debug.LogFormat("<color=#00ff00>[] Save dependency in {0} -> {1} </color>", DependenciesAssetPath, version);
