...
namespace Yodo1Unity
{
    public class SDKWindow_iOS : EditorWindow
    {
        ...
        private void DrawHeader()
        {
            ...
            if (questionMarkIcon != null && GUI.Button(new Rect(base.position.width - 35, 5, 30, 30), questionMarkIcon))
            {
                ...
                for (int i = 0; i < settings.configAnalytics.Count; i++)
                {
                    ...
                    switch (item.Index)
                    {
                        case (int)SettingsConstants.AnalyticsType.AppsFlyer:
                            ...
                            EditorGUIUtility.labelWidth = originalValue + 30;

                            settings.GetKeyItem().AppsFlyerDevKey = EditorGUILayout.TextField("AppsFlyer DevKey", settings.GetKeyItem().AppsFlyerDevKey, new GUILayoutOption[0]);
                            ...
                            settings.GetKeyItem().AppsFlyerOneLinkId = EditorGUILayout.TextField("AppsFlyer OneLinkId", settings.GetKeyItem().AppsFlyerOneLinkId, new GUILayoutOption[0]);
                            settings.GetKeyItem().AppsFlyer_identifier = EditorGUILayout.TextField("AppsFlyer identifier", settings.GetKeyItem().AppsFlyer_identifier, new GUILayoutOption[0]);
                            settings.GetKeyItem().AppsFlyer_Schemes = EditorGUILayout.TextField("AppsFlyer schemes", settings.GetKeyItem().AppsFlyer_Schemes, new GUILayoutOption[0]);
                            settings.GetKeyItem().AppsFlyer_domain = EditorGUILayout.TextField("AppsFlyer Associated-domains", settings.GetKeyItem().AppsFlyer_domain, new GUILayoutOption[0]);
                        ...
                        case (int)SettingsConstants.AnalyticsType.Thinking:
                            ...
                            settings.GetKeyItem().ThinkingAppId = EditorGUILayout.TextField("Thinking AppId", settings.GetKeyItem().ThinkingAppId, new GUILayoutOption[0]);
