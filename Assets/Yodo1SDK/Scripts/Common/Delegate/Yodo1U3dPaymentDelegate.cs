...
public class Yodo1U3dPaymentDelegate
{
    ...
    //    public static Yodo1U3dConstants.PayType payType = Yodo1U3dConstants.PayType.PayTypeChannel;
    ...
    //获取内购买验证票据
    public delegate void ValidatePaymentDelegate(string productId, Yodo1U3dIAPReceipt receipt);

    private static ValidatePaymentDelegate _validatePaymentDelegate;

    public static void setValidatePaymentDelegate(ValidatePaymentDelegate action)
    {
        _validatePaymentDelegate = action;
    }
    ...
    public static void OnDestroy()
    {
        ...
        _validatePaymentDelegate = null;
        _restorePurchasesDelegate = null;
    ...
    public static void Callback(int flag, int resultCode, Dictionary<string, object> obj)
    {
        switch (flag)
        {
...
#if UNITY_ANDROID
                if (_validatePaymentDelegate != null)
                {
                    Yodo1U3dIAPReceipt receiptData = null;
                    if (obj.ContainsKey("extra"))
                    {
                        receiptData = Yodo1U3dIAPReceipt.CreateReveipt_android(obj["extra"]);
                    }

                    _validatePaymentDelegate(uniformProductId, receiptData);
                }
#endif
            ...
            case Yodo1U3dSDK_ResulType_ValidatePayment:
                {
                    string uniformProductId = string.Empty; //商品ID
                    ...
                    if (_validatePaymentDelegate != null && !string.IsNullOrEmpty(uniformProductId))
                    {
                        Yodo1U3dIAPReceipt receiptData = null;
                        if (obj.ContainsKey("response"))
                        {
                            receiptData = Yodo1U3dIAPReceipt.CreateReveip_ios(obj["response"]);
                        }
    ...
    static Yodo1U3dProductData AddProductFromDictionary(Dictionary<string, object> productData)
    {
        ...
        {
            double.TryParse(productData["price"].ToString(), NumberStyles.Float, CultureInfo.InvariantCulture, out _price);
        }
    ...
    static void QuerySubscriptionsCallback(int reslutCode, Dictionary<string, object> obj)
    {
        if (reslutCode == 1)
        {
            ...
            {
                double.TryParse(obj["serverTime"].ToString(), NumberStyles.Float, CultureInfo.InvariantCulture, out serverTime);
            }
    ...
    static Yodo1U3dSubscriptionInfo AddSubscriptionInfoFromDictionary(Dictionary<string, object> subscriptionInfo)
    {
        ...
        {
            double.TryParse(subscriptionInfo["expiresTime"].ToString(), NumberStyles.Float, CultureInfo.InvariantCulture, out expiresTime);
        ...
        {
            double.TryParse(subscriptionInfo["purchase_date_ms"].ToString(), NumberStyles.Float, CultureInfo.InvariantCulture, out purchase_date_ms);
